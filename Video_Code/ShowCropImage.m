...
handles.color_code_current = handles.masterhandles.color_code_current;
...
for i = 1:numel(channel)
    ...
    if channel(i) ~= 4
        ...
        temp = imcrop(temp, handles.masterhandles.position);
        for j = 1:3
        ...
        if handles.masterhandles.rotation_angles(imageid) ~= 0
            temp = imrotate(temp, handles.masterhandles.rotation_angles(imageid), 'nearest');
        end
        if handles.masterhandles.mirror_image(imageid) ~= 0
            temp = fliplr(temp);
        end
...
if isfield(handles.masterhandles, 'fix_distortion_check')
    if get(handles.masterhandles.fix_distortion_check, 'Value')
...
hlow = plot(handles.imhist_axes, [max(display_range(1), x(1)) max(display_range(1), x(1))], [0 max(count)], ':', 'Color', hcolor);
hhigh = plot(handles.imhist_axes, [min(display_range(2), x(end)) min(display_range(2), x(end))], [0 max(count)], ':', 'Color', hcolor);
